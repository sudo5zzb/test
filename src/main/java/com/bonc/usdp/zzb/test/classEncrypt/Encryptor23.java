...
public class Encryptor23 {

    private static byte[] flags = "blockchain".getBytes();


    public static void main(String[] args) {
        String classDir = "D:\\gitRepo\\mgc-server\\target\\classes";
        try {
...
//        }
        ...
        for (int i = 8; i < bytes.length; i++) {
            encryptedBytes[i] = (byte) (bytes[i]);
        }
        //插入分隔符
        for (int i = 0; i < flags.length; i++) {
            encryptedBytes[encryptedBytes.length-10 + i] = flags[i];
        }
